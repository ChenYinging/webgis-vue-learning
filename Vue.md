...
![alt text](vue图片/图片42.png) 

Vue 的计算属性会自动追踪响应式依赖。它会检测到 publishedBooksMessage 依赖于 author.books，所以当 author.books 改变时，任何依赖于 publishedBooksMessage 的绑定都会同时更新。

## **计算属性缓存 vs 方法**

![alt text](vue图片/图片43.png) 
...
![alt text](vue图片/图片44.png) 

## **获取上一个值**

可以通过访问计算属性的 getter 的第一个参数来获取计算属性返回的上一个值：

![alt text](vue图片/图片45.png) 

 

如果你正在使用可写的计算属性的话：
![alt text](vue图片/图片46.png)
...
![alt text](vue图片/图片47.png) 

当不使用构建步骤时，一个 Vue 组件以一个包含 Vue 特定选项的 JavaScript 对象来定义：

![alt text](vue图片/图片48.png) 

## **使用组件**

要使用一个子组件，我们需要在父组件中导入它。假设我们把计数器组件放在了一个叫做 ButtonCounter.vue 的文件中，这个组件将会以默认导出的形式被暴露给外部。

![alt text](vue图片/图片49.png) 
...
Props 是一种特别的 attributes，你可以在组件上声明注册。这里要用到 [defineProps](#defineprops-defineemits) 宏：

![alt text](vue图片/图片50.png) 

defineProps 是一个仅 <script setup> 中可用的编译宏命令，并不需要显式地导入。声明的 props 会自动暴露给模板。defineProps 会返回一个对象，其中包含了可以传递给组件的所有 props：

![alt text](vue图片/图片51.png) 

 

如果你没有使用 <script setup>，props 必须以 props 选项的方式声明，props 对象会作为 setup() 函数的第一个参数被传入：

![alt text](vue图片/图片52.png) 